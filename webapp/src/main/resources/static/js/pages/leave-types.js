...
(function() {
    ...
    function load() {
        return Api.get('/api/leave-types').then(function(list) {
            ...
            for (var i = 0; i < list.length; i++) {
                ...
                rows.push([lt.id, lt.name, lt.code || '—', lt.sortOrder || 0, needAtt, lt.remark || '—', lt]);
            }
            ...
            html += UI.table(
                ['ID', '假别名称', '编号', '排序', '需佐证', '备注', '操作'],
                rows,
                [null, null, null, null, null, null, function(val, row) {
                    var lt = row[6];
    ...
    window.ltEdit = function(id) {
        var initData = { name: '', code: '', sortOrder: 0, needAttachment: false, remark: '' };
